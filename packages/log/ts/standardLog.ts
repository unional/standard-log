...
export function createStandardLog<N extends string = LogMethodNames>(
  ...
  if (options?.reporters?.some(r => !r.id)) {
    ...
    log.critical(`Detected reporter with empty id. This is not allowed as it will create problems with feature like 'getNonConsoleLogger()'. Please add an id to your reporter.`)
  }
...
export function createStandardLogClosure<N extends string = LogMethodNames>(
  ...
  const logMethods = DEFAULT_LOG_METHOD_NAMES
    .concat(Object.keys(options?.customLevels ?? {}))
    .concat(['on', 'count', 'write'])
  return {
    store,
    standardLog: {
      ...
      getNonConsoleLogger(id, options?) {
        ...
        return store.nonConsoleLoggers[id] = nonConsoleReporters.length === 0
