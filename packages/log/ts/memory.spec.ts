...
const mikuIsSinging = {
	...
	args: ['miku', 'is', 'singing'],
	timestamp: new Date()
}
const lukaIsDancing = {
	id: 'test-log',
	level: logLevels.warn,
	args: ['luka', 'is', 'dancing'],
	timestamp: new Date()
}
...
describe(createMemoryLogReporter.name, () => {
	...
	describe(reporter.getLogMessages.name, () => {
		it('maps each log entries to string', () => {
			...
			expect(r.getLogMessages()).toEqual(['miku is singing', 'luka is dancing'])
		})
	})
	...
	describe(reporter.emit.name, () => {
		it('emit saved logs to console', () => {
			...
			r.write({
				id: 'test-log',
				level: logLevels.info,
				args: ['the two messages above are expected'],
				timestamp: new Date()
			})
